...
curl -XGET "localhost:9200/multimedia/_search?pretty" -H 'Content-Type: application/json' -d @scripts/query1.json > results/query1.json

# query2
# Directors	with the most action films
curl -XGET "localhost:9200/multimedia/_search?pretty" -H 'Content-Type: application/json' -d @scripts/query2.json > results/query2.json

# query3
# LGBTQ-themed films available in the collection. In addition to the listing, show the number of films by year
curl -XGET "localhost:9200/multimedia/_search?pretty" -H 'Content-Type: application/json' -d @scripts/query3.json > results/query3.json
